...
public class AudioUtil {
	 ...
	 * 得到pcm文件的毫秒数
	 *
	 * pcm文件音频时长计算 同图像bmp文件一样，pcm文件保存的是未压缩的音频信息。 16bits
	 * 编码是指，每次采样的音频信息用2个字节保存。可以对比下bmp文件用分别用2个字节保存RGB颜色的信息。 16000采样率 是指 1秒钟采样
	 * 16000次。常见的音频是44100HZ，即一秒采样44100次。 单声道： 只有一个声道。
	 *
	 * 根据这些信息，我们可以计算： 1秒的16000采样率音频文件大小是 2*16000 = 32000字节 ，约为32K 1秒的8000采样率音频文件大小是
	 * 2*8000 = 16000字节 ，约为 16K
	 *
